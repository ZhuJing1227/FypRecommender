...
for i in range(len(keys)):
    ...
    sim_mat_dic[keys[i]]=singapore_similarity_matrix



def recommend(Gender,Age_Group,singapore,sim_mat,top_n):
    sim_mat=sim_mat[(Gender,Age_Group)]#get which gender and agegroup then see which model to use
    idx=np.where(singapores==singapore)[0][0]# get the singapore and look through
    ...
    print(singapores[sorted_array[:top_n]])
    return singapores[sorted_array[:top_n]]